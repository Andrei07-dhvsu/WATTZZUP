    ...
    if ($response !== false && is_array($submeterData)) {
        foreach ($submeterData as $submeter) {
            if (in_array($submeter['submeterId'], $assignedSubmeters)) {
                ...
                $stmt = $pdo->prepare("SELECT * FROM consumption WHERE submeter_id = :submeter_id AND consumption_date = :currentDate");
                $stmt->execute([
                ...
                if (!$row) {
                    ...
                    $stmtInsert = $pdo->prepare("INSERT INTO consumption (submeter_id, consumption_date, kwh_start, kwh_end) 
                    ...
                    $stmtUpdate = $pdo->prepare("UPDATE consumption SET kwh_end = :kwh_end 
